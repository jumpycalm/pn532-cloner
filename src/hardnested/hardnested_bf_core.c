...
// MAX_BITSLICES for non-SIMD instruction set is 64
    ...
    const uint8_t *bf_test_nonce_2nd_byte, noncelist_t *nonces)
{
  ...
  for (uint32_t *restrict p_even = p->states[EVEN_STATE]; p_even < p_even_end; p_even += MAX_BITSLICES) {
    ...
    bitsliced_even_feedback[bitsliced_blocks] = lstate_p[(47 - 0) / 2].value ^ lstate_p[(47 - 10) / 2].value ^ lstate_p[(47 - 12) / 2].value ^ lstate_p[(47 - 14) / 2].value ^ lstate_p[(47 - 24) / 2].value ^ lstate_p[(47 - 42) / 2].value;
    bitsliced_blocks++;
  }
    ...
    for (uint32_t block_idx = 0; block_idx < bitsliced_blocks; ++block_idx, p_even += MAX_BITSLICES) {
      ...
      for (uint32_t tests = 0; tests < nonces_to_bruteforce; ++tests) {
        ...
        for (int32_t ks_idx = KEYSTREAM_SIZE - 1 - common_bits; ks_idx >= 0; --ks_idx) {
          ...
          if (ks_idx != 0) {
            fb_bits = (state_p[47 - 0].value ^ state_p[47 - 5].value ^ state_p[47 - 9].value ^ state_p[47 - 10].value ^ state_p[47 - 12].value ^ state_p[47 - 14].value ^ state_p[47 - 15].value ^ state_p[47 - 17].value ^ state_p[47 - 19].value ^ state_p[47 - 24].value ^ state_p[47 - 25].value ^ state_p[47 - 27].value ^ state_p[47 - 29].value ^ state_p[47 - 35].value ^ state_p[47 - 39].value ^ state_p[47 - 41].value ^ state_p[47 - 42].value ^ state_p[47 - 43].value);
          }
          // remember feedback and keystream vectors for later use
          uint8_t bit = KEYSTREAM_SIZE - ks_idx;
          // TODO: Compare with the PM3
