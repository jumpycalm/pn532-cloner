...
static uint8_t defaultKeys[][6] = {
  ...
  { 0xa0, 0xa1, 0xa2, 0xa3, 0xa4, 0xa5 }, // NFCForum MAD Key A (read key)
  { 0xb0, 0xb1, 0xb2, 0xb3, 0xb4, 0xb5 }, // MAD Key B (write key)
  { 0x89, 0xec, 0xa9, 0x7f, 0x8c, 0x2a }, // MAD Key B (write key)
...
static uint8_t specialKeys[][6] = {
  ...
  { 0x2a, 0x2c, 0x13, 0xcc, 0x24, 0x2a }, // Dormakaba Sector 1 Key A, Key B is factory default key
  { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff }, // Dormakaba Sector 2-15 Key A place holder (to be set based on UID), Key B is factory default key
...
// Thanks to https://gitee.com/jadenwu/Saflok_KDF/blob/master/saflok.c
static void set_dormakaba_key(uint8_t *uid)
{
...
#define KEY_LENGTH 6
  ...
  specialKeys[3][0] = magic_byte;
  specialKeys[3][1] = uid[0];
  specialKeys[3][2] = uid[1];
  specialKeys[3][3] = uid[2];
  specialKeys[3][4] = uid[3];
  specialKeys[3][5] = magic_byte;

  uint8_t carry_sum = 0;

  for (int i = KEY_LENGTH - 1; i >= 0 && magickal_index >= 0; i--, magickal_index--) {
    uint16_t keysum = specialKeys[3][i] + magic_table[magickal_index];
    specialKeys[3][i] = (keysum & 0xFF) + carry_sum;
    carry_sum = keysum >> 8;
  }
}
...
bool read_mfc()
{
  ...
  // Test special keys
  set_dormakaba_key(t.nt.nti.nai.abtUid);
